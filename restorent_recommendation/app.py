...
def recommend(name, cosine_similarities=cosine_similarities):
    # Create a list to store top restaurant names
    recommend_restaurant = []
    
    # Find the index of the restaurant entered
    idx = indices[indices == name].index[0]
    
    # Find restaurants with similar cosine similarity and order them
    score_series = pd.Series(cosine_similarities[idx]).sort_values(ascending=False)
    
    # Extract top 30 restaurant indexes
    top30_indexes = list(score_series.iloc[0:31].index)
    
    # Store the names of the top 30 restaurants
    for each in top30_indexes:
        recommend_restaurant.append(list(df_percent.index)[each])
    
    # Creating new DataFrame with restaurant names
    df_new = pd.DataFrame(columns=['restaurant', 'cuisines', 'Mean Rating', 'cost'])
    
    # Populate DataFrame with top 30 similar restaurants
    for each in recommend_restaurant:
        row = df_percent[df_percent.index == each].sample()
        row.insert(0, 'restaurant', each)  # Add restaurant name
        df_new = pd.concat([df_new, row], ignore_index=True)  # Use concat instead of append
    
    # Remove duplicates and sort by highest rating
    df_new = df_new.drop_duplicates(subset=['restaurant', 'cuisines', 'Mean Rating', 'cost'], keep=False)
...
def get_recommendation():
    ...
    recommendations = recommend(restaurant_name)  # Assign value first

    if recommendations is None or len(recommendations) == 0:
        return jsonify({
            "restaurant": restaurant_name,
            "error": f"No data found for restaurant: '{restaurant_name}'"
        }), 404  # Clearer message with restaurant name

    try:
        recommendations_json = recommendations.to_dict(orient="records")  # Ensure `recommendations` is a DataFrame
        return jsonify({
            "restaurant": restaurant_name,
        ...
        }), 500  # More detailed error response
